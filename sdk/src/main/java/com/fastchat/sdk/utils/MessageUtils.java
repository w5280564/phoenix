...
public class MessageUtils {
    ...
    public static void handleReceiveMessage(Message message, Context context, boolean isOffline) {
        ...
        try {
            ...
            //自定义消息
            if (type == SDKConstant.TYPE_MESSAGE_CMD) {//处理CMD消息
                ExtensionElement timeExtensionElement = message.getExtension("urn:xmpp:delay");
            ...
            } else if (type == SDKConstant.TYPE_MESSAGE_HT) {//处理正常消息体
                ...
                if (action == 2000) {
                    ...
                    createNewGroup(htMessage.getFrom(), htMessage.getTo(), groupName, groupDesc, groupAvatar);
                } else if (action == 2004) {
               /*  有群成员被踢的消息
...
//                    extJson.put("owner",htGroup.getOwner());
                    createNewGroup(htMessage.getStringAttribute("owner"),htMessage.getUsername(),htMessage.getStringAttribute("groupName"),
...
//                    HTClient.getInstance(). messageManager().saveMessage(htMessage, true);
            ...
            } else if (type == SDKConstant.TYPE_MESSAGE_GROUP_DESTROY) {
           /*      群被删除,通知群成员;
            ...
            } else if (type == SDKConstant.TYPE_MESSAGE_GROUP_LEAVE) {
...
//                sendNewMessageBroadcast(htMessage,context);
            ...
            }else if(type == SDKConstant.TYPE_CALL){
                ExtensionElement time = message.getExtension("delay", "urn:xmpp:delay");
...
//        context.sendBroadcast(intent);
    ...
    private static void createNewGroup(String owner, String groupId, String groupName, String groupDesc, String imgUrl) {

        HTGroup htGroup = new HTGroup();
        htGroup.setOwner(owner);
        htGroup.setGroupName(groupName);
        htGroup.setGroupImId(groupId);
        htGroup.setGroupHeadImg(imgUrl);
    ...
    public static String getMsgId(String xmlString) {
        if (xmlString.contains("id='")) {
            String[] strings = xmlString.split("id='");
            if (strings.length > 1) {
                String str1 = strings[1];
                if (str1.contains("'/>")) {
    ...
    public static String getMsgType(String xmlString) {
        if (xmlString.contains("xmlns='urn:xmpp:receipts'")) {
            String[] strings = xmlString.split("xmlns='urn:xmpp:receipts'");
            if (strings.length > 0) {
