...
//    }
    ...
    public static boolean saveUriPermission(Context context, Uri fileUri, Intent intent) {
        ...
        try {
            ...
            LogUtil.d(TAG, "saveUriPermission last part of Uri: "+last);
        } catch (SecurityException e) {
            LogUtil.e("EaseFileUtils", "saveUriPermission failed e: "+e.getMessage());
        }
        if(!TextUtils.isEmpty(last)) {
            FastPreferenceManager.getInstance().putString(last, fileUri.toString());
            return true;
        }
        return false;
    }
    ...
    public static Uri takePersistableUriPermission(Context context, Uri uri) {
        ...
        if(!TextUtils.isEmpty(last)) {
            String fileUri = FastPreferenceManager.getInstance().getString(last);
            if(!TextUtils.isEmpty(fileUri)) {
                try {
                    ...
                    LogUtil.e("EaseFileUtils", "takePersistableUriPermission failed e: "+e.getMessage());
                    return null;
                }
            }
        }
