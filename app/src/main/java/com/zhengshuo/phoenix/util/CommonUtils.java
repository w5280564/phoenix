...
import com.zhengshuo.phoenix.ui.dialog.TipsAlertDialog;
...
//            return;
//        }
//        Glide.with(context).load(object).placeholder(R.mipmap.default_image).diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
...
//        LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent(IMAction.ACTION_HAS_CANCLED_NO_TALK).putExtra("userId", groupId).putExtra("content", content));
    ...
    public static void loadGroupAvatar(Context context, Object object, ImageView imageView) {
        ...
        Glide.with(context).load(object).placeholder(R.mipmap.default_group).diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
