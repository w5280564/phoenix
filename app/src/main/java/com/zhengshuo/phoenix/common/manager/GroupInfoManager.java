...
/**
 * Created by ouyang on 2019/7/31.
 * 
 */
public class GroupInfoManager {
    ...
    public void saveGroupManager(String groupId, JSONArray jsonArray) {
        MmvkManger.getInstance().putJSONArray(groupId + "_managers", jsonArray);

    }

    public JSONArray getGroupManagers(String groupId) {
        JSONArray jsonArray = MmvkManger.getInstance().getJSONArray(groupId + "_managers");
        if (jsonArray == null) {
    ...
    public void setsilentUsers(String groupId, JSONArray jsonArray) {
        ...
        MmvkManger.getInstance().putJSONArray(groupId + "_silentUser", jsonArray);
        if(jsonArray!=null){
            for(int i=0;i<jsonArray.size();i++){
                JSONObject jsonObject=jsonArray.getJSONObject(i);
    ...
    public JSONArray getsilentUsers(String groupId) {
        ...
        JSONArray jsonArray = MmvkManger.getInstance().getJSONArray(groupId + "_silentUser");
        if (jsonArray == null) {
            jsonArray = new JSONArray();
        }
        return jsonArray;
...
//         });//todo
    ...
    public boolean isGroupSilent(String groupId) {
        boolean isSilent=MmvkManger.getInstance().getBoolean(groupId + "_GroupSilent", false);
        boolean isSilent_Single=MmvkManger.getInstance().getBoolean(groupId + "_GroupSilent_Single", false);
    ...
    public void setGroupSilent(String groupId, boolean silent,boolean isSingle) {
        if(!isSingle){
            MmvkManger.getInstance().putBoolean(groupId + "_GroupSilent", silent);
        }else {
            MmvkManger.getInstance().putBoolean(groupId + "_GroupSilent_Single", silent);
    ...
    public void setGroupRole(String groupId, int roleType) {
        MmvkManger.getInstance().putInt(groupId + "_GroupRole", roleType);

        // groupRoleMap.put(groupId, roleType);
    }

    public boolean isManager(String groupId) {
        int roleType = MmvkManger.getInstance().getInt(groupId + "_GroupRole", 0);
...
//        });
    ...
    public JSONArray getGroupAllMembersFromLocal(String groupId) {
        JSONArray jsonArray = MmvkManger.getInstance().getJSONArray(groupId + "_allMembers");
        if (jsonArray == null) {
            jsonArray = new JSONArray();
        }
        return jsonArray;
    ...
    public Set<String> getAllMemberUserIdFromLocal(String groupId) {
        Set<String> stringSet = MmvkManger.getInstance().getStringSet(groupId + "_allMembers_userId");
        if (stringSet == null) {
    ...
    public void removMember(String groupId, String membersId) {
        ...
        if (jsonArray != null) {
            ...
            MmvkManger.getInstance().putJSONArray(groupId + "_allMembers", jsonArray);

        }

        Set<String> allUserIds = getAllMemberUserIdFromLocal(groupId);
        allUserIds.remove(membersId);
        MmvkManger.getInstance().putStringSet(groupId + "_allMembers_userId", allUserIds);
    ...
    public void setAtTag(String groupId,boolean isAt){
        MmvkManger.getInstance().putBoolean(groupId + "_atMsg", isAt);
    }

    public boolean getAtTag(String groupId){
        return    MmvkManger.getInstance().getBoolean(groupId + "_atMsg", false);
    }
